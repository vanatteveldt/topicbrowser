...
plot.wordcloud.time <- function(m, docsums, date_var, topic_nr, date_interval='year', value='relative', wordsize_scale=0.5){
  par(mar = c(4.5, 3, 2, 1), cex.axis = 1.7)
  layout(matrix(c(1, 1, 2, 2), 2, 2, byrow = TRUE), widths = c(2.5, 1.5), heights = c(1, 2))  
  topics.plot.time(docsums, topic_nr = topic_nr, time_var = date_var, date_interval = date_interval, value = value)
  topics.plot.wordcloud(posterior(m)$terms, topic_nr = topic_nr, wordsize_scale = wordsize_scale)
