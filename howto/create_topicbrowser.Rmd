...
```{r}
info = clusterinfo(m=sotu.m, terms=sotu.tokens$lemma, documents=sotu.tokens$aid, meta=sotu.meta)
names(info)
```
...
```{r, message=F}
output = createTopicBrowser(info)
```
...
pf.wordcloud(info, topic_nr=2) # only wordcloud
pf.time(info, topic_nr=2) # only time graph
```
...
```{r}
pf.semnet(info, topic_nr=1) # requires semnet package
```
...
```{r, message=FALSE}
createTopicBrowser(info, plotfunction.pertopic=list(pf.wordcloud_time, pf.semnet))
```
